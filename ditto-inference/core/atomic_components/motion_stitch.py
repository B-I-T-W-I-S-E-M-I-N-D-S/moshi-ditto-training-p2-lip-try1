...
class MotionStitch:
    ...
    def setup(
        ...
        freeze_head_pose=False,  # NEW: freeze pitch/yaw/roll/t to source pose
    ):
        self.is_image_flag = is_image_flag
        self.freeze_head_pose = freeze_head_pose
        if use_d_keys is None:
            if self.is_image_flag:
                if self.freeze_head_pose:
                    # Only drive expression; all pose keys come from source frame.
                    self.use_d_keys = ("exp",)
                else:
